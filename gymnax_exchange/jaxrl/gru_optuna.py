...
def objective(trial):
    ...
    print(np.isnan(out['metric'][0]["average_price"]).any())
    print(np.isnan(out['metric'][0]["total_revenue"]).any())
    print(out['metric'][0]["average_price"])
    print(type(out['metric'][0]["average_price"]))
    print(out['metric'][0]["total_revenue"])
    print(type(out['metric'][0]["total_revenue"]))
    # Define your evaluation metric
    evaluation_metric = np.nanmean(out['metric'][0]["average_price"])
    return evaluation_metric

if __name__ == "__main__":
    storage_name = "sqlite:///optuna_study_gru.db"
    study = optuna.create_study(direction="maximize", storage=storage_name, study_name="gru_ppo", load_if_exists=True)
