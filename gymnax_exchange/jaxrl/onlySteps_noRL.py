...
def make_train(config):
    ...
    '''
    Adjusted by Gera 06.24 14:00
    Adding window_index parameter to the inputs
    Original code was:
    env= ExecutionEnv(config["ATFOLDER"],config["TASKSIDE"],config["DEBUG"])
    
    '''
    env= ExecutionEnv(config["ATFOLDER"],config["TASKSIDE"],config["WINDOW_INDEX"],config["DEBUG"])
...
if __name__ == "__main__":
    try:
        ATFolder = sys.argv[1] 
        print('system_infooooooooooo:',sys.argv[1])
    except:
